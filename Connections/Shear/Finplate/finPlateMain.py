...
class MainController(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.actionSave.triggered.connect(self.saveDesign_inputs)
        self.ui.actionOpen_inputs.triggered.connect(self.openDesign_inputs)
        #graphics
...
#             self.ui.comboWldSize.setCurrentIndex(self.ui.comboWldSize.findText(str(uiObj['Weld']['Size (mm)'])))
    ...
    def openDesign_inputs(self):
        ...
        fileName = QtGui.QFileDialog.getOpenFileName(self,"Open Design",'output/finplate/Design/',"All Files(*)")
        if not fileName:
            return
        try:
    ...
    def save_inputs(self,uiObj):
        ...
        '''
        inputFile = 'Connections/Shear/Finplate/saveINPUT.txt'
        try:
            jsondata = json.dumps(uiObj, indent=4, skipkeys=True, sort_keys=True)
            fd = open(inputFile, 'w')
            fd.write(jsondata)
            fd.close()
        except:
            print 'ERROR writing', inputFile
            pass 
